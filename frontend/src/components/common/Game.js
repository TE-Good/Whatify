...
export default class Game extends React.Component {
  constructor() {
    ...
    this.score = 0  //score = accumulated points over the game
    this.points = 0 //points = points for current round to acc to score
    this.round = 0
  ...
  handleAnswers() {
    ...
    //   this.state.gameData[(Math.ceil(Math.random() * Math.ceil(this.state.gameData.length - 1)))].track_name)



      //the right answer is always = gameData[round].track_name
  ...
  render() {
    ...
    return (
      <>
        <div>
          {!this.round ? <h2 className='' id='r2p'>Get Ready To Play!</h2> : <h2>Round {this.round}</h2>}
        ...
        <div className='stage'>
          ...
          <div className='hidden'>
            <iframe src={this.state.gameData[this.round].track_preview} allow='autoplay' />
          </div>
          ...
          {!this.round ? <button onClick={this.nextRound} value='1' >Start Game</button> : <button onClick={this.nextRound} value='1' >Next Round</button>}
        </div> {/* end of stage */}

        <small className='bottom'>answer = {this.state.gameData[this.round].track_name}</small>
      </>
    )
  }
}